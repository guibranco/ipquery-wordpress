	...
	<table class="widefat striped ipquery-visitors-table">
		...
		<?php else : ?>
			...
			foreach ( $rows as $row ) :
				$flags = array();
				if ( $row['is_vpn'] ) {
					$flags[] = '<span class="ipquery-badge ipquery-badge--orange">VPN</span>';
				}
				if ( $row['is_proxy'] ) {
					$flags[] = '<span class="ipquery-badge ipquery-badge--red">Proxy</span>';
				}
				if ( $row['is_tor'] ) {
					$flags[] = '<span class="ipquery-badge ipquery-badge--red">Tor</span>';
				}
				if ( $row['is_datacenter'] ) {
					$flags[] = '<span class="ipquery-badge ipquery-badge--blue">DC</span>';
				}
				if ( $row['is_mobile'] ) {
					$flags[] = '<span class="ipquery-badge ipquery-badge--green">Mobile</span>';
				}

				$score     = (int) $row['risk_score'];
				$score_cls = $score >= 80 ? 'red' : ( $score >= 40 ? 'orange' : 'green' );
				$location  = array_filter( array( $row['city'], $row['state'], $row['country'] ) );
				?>
			<tr>
